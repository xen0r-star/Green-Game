...
class custom_Button(Frame):
    """
        Classe pour un widget de bouton personnalisé. C'est un bouton dans lequel on peut mettre une image.
        Cela permet de ne pas répéter plusieurs fois ce code, souvent utilisé pour créer des boutons
        avec des apparences différentes dans le programme.
    """
    ...
    def create_buttons(self):
        def buttonClicked():
            if self.command and callable(self.command):  # callable => Vérifie si une variable est appelable (fonction)
                self.command()
